...
<b>Ushbu malumotlar tog'riligiga ishonch hosil qiling<tg-emoji emoji-id="5406631276042002796">📨</tg-emoji></b>""",reply_markup=tas,parse_mode="HTML")
...
async def tasdiq(call:CallbackQuery,state:FSMContext):
    ...
    await state.clear()
    limit_sec = 3600
    start_time = time.time()  # Sikl boshlanish vaqti
...
Qolgan {qoldi} limitingiz qaytarildi!""")
                async with async_session() as session:
                    ...
                    await session.commit()
                break
            holat = await send_sms(phone=nomer,formatted_phone=formatted,message=call.message)
            if holat:
                qoldi = qoldi - 1
